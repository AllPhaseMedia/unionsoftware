...
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    // Verify step belongs to the case
    const existingStep = await prisma.disciplinaryStep.findFirst({
      where: {
        id: stepId,
        caseId: id,
      },
    ...
    const step = await prisma.disciplinaryStep.update({
      where: { id: stepId },
      data: {
    ...
    if (shouldRecalculate) {
      ...
      if (status === "PENDING" && completedAt === null) {
        ...
        const previousCompletedStep = await prisma.disciplinaryStep.findFirst({
          where: {
            caseId: id,
        ...
        if (previousCompletedStep && previousCompletedStep.completedAt) {
          ...
          baseDate = new Date(disciplinaryCase.filingDate);
          startFromStepNumber = 0;
        }

        // Get all steps that need recalculation (from the reset step onwards)
        const stepsToRecalculate = await prisma.disciplinaryStep.findMany({
          where: {
            caseId: id,
        ...
        for (const stepToUpdate of stepsToRecalculate) {
          ...
          if (defaultDays) {
            ...
            await prisma.disciplinaryStep.update({
        ...
        const subsequentSteps = await prisma.disciplinaryStep.findMany({
          where: {
            caseId: id,
