...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    if (documentId) {
      const document = await prisma.disciplinaryDocument.findUnique({
        where: { id: documentId },
      });
    ...
    const documents = await prisma.disciplinaryDocument.findMany({
      where: { caseId: id },
    ...
    console.error("Error fetching disciplinary documents:", error);
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const storagePath = `${dbUser.organization.slug}/disciplinary/${id}/${fileName}`;
    ...
    const document = await prisma.disciplinaryDocument.create({
      data: {
        caseId: id,
        fileName: file.name,
        fileType: file.type,
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    // Delete record
    await prisma.disciplinaryDocument.delete({
      where: { id: documentId },
    });
