...
export function DisciplinaryDiscussionSection({
  messages,
  caseId,
  currentUserId,
  onMessageSent,
  ...
  const sendMessage = async (content: string, parentId?: string | null) => {
    ...
    setIsSending(true);
    try {
      const response = await fetch(`/api/disciplinary/${caseId}/messages`, {
        method: "POST",
  ...
  const renderMessage = (message: DisciplinaryMessage, depth = 0) => {
