...
interface MemberDetailProps {
  initialMember: MemberWithRelations;
}

export function MemberDetail({ initialMember }: MemberDetailProps) {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const response = await fetch(`/api/members/${member.id}/notes`, {
        method: "POST",
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/members/${member.id}/documents`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleDeleteDocument = async (documentId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/members/${member.id}/documents?documentId=${documentId}`,
        { method: "DELETE" }
      );
