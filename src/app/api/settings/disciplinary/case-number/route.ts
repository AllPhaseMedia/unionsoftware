...
export interface DisciplinaryCaseNumberSettings {
  prefix: string;
  includeYear: boolean;
  separator: string;
  nextNumber: number;
...
const DEFAULT_SETTINGS: DisciplinaryCaseNumberSettings = {
  prefix: "DC",
  includeYear: true,
  separator: "-",
  nextNumber: 1,
...
export async function GET() {
  try {
    ...
    const caseNumberSettings: DisciplinaryCaseNumberSettings = { ...DEFAULT_SETTINGS };
    ...
    console.error("Error fetching disciplinary case number settings:", error);
...
export async function PUT(request: Request) {
  try {
    ...
    const { prefix, includeYear, separator, nextNumber, padding } = body as DisciplinaryCaseNumberSettings;
    ...
    const settingsToSave = [
      { key: "disciplinaryCaseNumber.prefix", value: prefix },
      { key: "disciplinaryCaseNumber.includeYear", value: String(includeYear) },
      { key: "disciplinaryCaseNumber.separator", value: separator },
      { key: "disciplinaryCaseNumber.nextNumber", value: String(nextNumber) },
      { key: "disciplinaryCaseNumber.padding", value: String(padding) },
    ];
    ...
    console.error("Error saving disciplinary case number settings:", error);
    return NextResponse.json(
